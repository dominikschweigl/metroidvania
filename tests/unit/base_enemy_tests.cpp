...
class TestEnemy : public BaseEnemy {
  public:
	TestEnemy(sf::Vector2f spawn, float w = 32.f, float h = 32.f)
		: BaseEnemy(spawn, w, h) {}
...
class TestState : public EnemyState {
  public:
	EnemyState *nextState =
		nullptr; // if non-null and != this, returned on update
	int updateCalls = 0;
	int animationCalls = 0;
	int enterCalls = 0;
	int exitCalls = 0;

	EnemyState *update(float dt, BaseEnemy &e, const World &w,
...
TEST_CASE("applyGravity with custom gravity values") {
	...
	SECTION("Higher gravity accelerates faster") {
		...
		EntityPhysics::applyGravity(velY, onGround, 0.1f, customGravity, bounds,
									w);
...
TEST_CASE("BaseEnemy respects custom gravity field") {
	...
	SECTION("Default gravity is 1200") { REQUIRE(e.gravity == 1200.f); }
...
TEST_CASE("resolveVertical plants enemy on floor and sets isOnGround") {
	...
	float y = EntityPhysics::resolveVertical(pos, velY, onGround, 28.f, 28.f,
											 0.1f, w);
