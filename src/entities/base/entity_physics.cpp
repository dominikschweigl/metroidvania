...
bool isGroundBelow(sf::FloatRect bounds, const World &world) {
	...
	auto right = world.getTileAtCoordinate(
		{bounds.position.x + bounds.size.x, bott + 1.f});
	return (left.has_value() && left.value()->isSolid) ||
		   (right.has_value() && right.value()->isSolid);
}
...
float resolveHorizontal(sf::Vector2f pos, float &velX, float width,
	...
	sf::FloatRect bounds({pos.x - width / 2.f, pos.y - height},
						 {width, height});

	sf::FloatRect future({futureX - width / 2.f, bounds.position.y},
						 {width, height});
	if (world.isSolidAtRect(future)) {
		...
		if (tile.has_value()) {
			if (deltaX >= 0.f) {
				return tile.value()->position.x + World::TILE_SIZE -
					   width / 2.f - 1.f;
			}
...
float resolveVertical(sf::Vector2f pos, float &velY, bool &isOnGround,
	...
	sf::FloatRect future({bounds.position.x, futureY - height},
						 {width, height});
	if (world.isSolidAtRect(future)) {
...
void simulateMovement(float deltaTime, sf::Vector2f &position,
	...
	for (float t = 0.f; t < deltaTime; t += step) {
		applyGravity(
			velocity.y, isOnGround, step, gravity,
			sf::FloatRect({position.x - width / 2.f, position.y - height},
						  {width, height}),
			world);

		position.x =
			resolveHorizontal(position, velocity.x, width, height, step, world);
		position.y = resolveVertical(position, velocity.y, isOnGround, width,
									 height, step, world);
	}
}
} // namespace EntityPhysics
